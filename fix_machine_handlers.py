...
handlers_mock = """func (s *Server) desk(w http.ResponseWriter, r *http.Request) { w.Write([]byte(`{"status":"ok"}`)) }
...
real_handlers = """func (s *Server) desk(w http.ResponseWriter, r *http.Request) {
	s.machine(w, r)
}
...
if handlers_mock in text:
    text = text.replace(handlers_mock, real_handlers)
    with open("internal/server/server.go", "w", encoding="utf-8") as f:
        f.write(text)
    print("REPLACED!")
else:
    print("MOCKS NOT FOUND!")

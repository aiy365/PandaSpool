...
function card(inner) {
  return `<section class="card bg-base-100 shadow-sm border border-base-300">${inner.includes("card-body") ? inner : `<div class="card-body">${inner}</div>`}</section>`;
}
...
async function viewHome() {
  ...
  const mqtt = m.error
    ...
    : m.connected
      ...
      : `<span class="badge badge-ghost">未连接拓竹，去设置页填写</span>`;
...
async function viewCompare() {
  ...
  $("#page").innerHTML = card(`
    <h1 class="card-title">参数横评</h1>
    <p class="muted">已确认条目。烘干 / 喷嘴 / 热床 / 速度排在前面。同一字段多个值标成冲突，不覆盖。</p>
    ${keys.length === 0 ? "<p>先在产品里记几条已确认参数。</p>" : keys.map((k) => {
      const uniq = new Set(data[k].map((c) => c.value + "|" + c.unit));
      const hit = uniq.size > 1;
      return `<h2 class="card-title text-base">${esc(k)} ${hit ? '<span class="badge badge-error">冲突</span>' : ""}</h2>
      <div class="overflow-x-auto"><table class="table table-zebra">
        <thead><tr><th>产品</th><th>来源</th><th>值</th></tr></thead>
        <tbody>${data[k].map((c) => `<tr><td>${esc(c.product)}</td><td><span class="badge badge-ghost">${esc(c.source)}</span></td><td>${esc(c.value)} ${esc(c.unit)}</td></tr>`).join("")}</tbody>
      </table></div>`;
    }).join("")}
  `);
}
...
function swRow(label, role) {
  return `<div class="sw"><span>${label}</span>
    <span class="join">
      <button type="button" class="btn btn-sm join-item btn-ghost" data-a="${role}" data-on="0">关</button>
      <button type="button" class="btn btn-sm join-item btn-primary" data-a="${role}" data-on="1">开</button>
    </span></div>`;
}
...
async function viewMachine() {
  ...
  const ensureShell = () => {
    ...
    $("#mach").onclick = async (e) => {
      const btn = e.target.closest("[data-a]");
      if (!btn) return;
      await busy(btn, async () => {
        await api("/api/actuators/" + btn.dataset.a, { method: "POST", body: { on: btn.dataset.on === "1" } });
  ...
  const draw = async () => {
    ...
    $("#mach-stats").innerHTML = `
      ...
      <p class="muted">层数 ${b.layer ?? "—"} / ${b.total_layer ?? "—"}　${esc(b.subtask || "")}</p>`;
    let airTs = Number(air.ts);
    if (airTs > 1e12) airTs /= 1000;
    ...
    if (d.ezviz?.configured && !window.__ez && window.EZUIKit && $("#ezviz")) {
      try {
      ...
      } catch (e) { toast(e.message, "error", { id: "mach" }); }
    }
  };
  await draw();
...
async function viewSettings(me) {
  ...
  $("#page").innerHTML = `
    ...
    `)}</div>
    <div class="settings-webhook">${card(`
  ...
  const collect = () => ({
    ...
    ezviz: { app_key: $("#zk").value, app_secret: $("#zs").value, device_serial: $("#zd").value, channel: $("#zc").value },
    air: { token: $("#at").value },
    ai: { token: $("#ait")?.value || "" },

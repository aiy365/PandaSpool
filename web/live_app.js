    ...
    ${card(`
      ...
      <div class="overflow-x-auto"><table class="table table-zebra">
        ...
        <tbody>${[...(p.colors || [])].sort((a, b) => Number(colorOnShelf(b)) - Number(colorOnShelf(a)) || (b.unopened - a.unopened)).map((c) => {
          ...
          <td class="join"><button class="btn btn-primary btn-xs join-item" data-intakec="${c.id}">入库</button><button class="btn btn-ghost btn-xs join-item" data-savec="${c.id}">存</button><button class="btn btn-ghost btn-xs join-item" data-delc="${c.id}">删</button></td></tr>`;
        }).join("")}</tbody>
      </table></div>
    `)}
    ${card(`
    ...
    try {
      if (btn.dataset.intakec) {
        ...
        try {
          ...
          const codes = spools.slice(-qty).map(s => s.short_code).join(", ");
          alert("入库成功！请用记号笔在料盘上写下以下编号：\n\n" + codes);
          ...
          <tbody id="spools-body">
            ${filtered.map(s => {
              ...
              <tr>
                ...
                <td>
                  <div class="font-bold">${esc(fname)}</div>
